...
namespace CustomCharacterLoader.Patches
{
    ...
    public static class GetCueSheetPatch
    {
        ...
        {
            if(Main.playerLoader.playerType != PlayerManager.PlayerLoader.CharacterType.None && Main.playerLoader.selectedCharacter.monkey_acb != "")
            {
                return (sound_id.cuesheet) 6969;
            }
            else
            {
